// =============================================
// LOKEA - Page Détail Appartement
// =============================================
import React, { useState, useEffect } from 'react';
import { useParams, Link } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import {
  MapPin, Users, BedDouble, Bath, Wifi, ChevronLeft,
  ChevronRight, X, Play, ArrowLeft, Eye, CheckCircle
} from 'lucide-react';
      ...
      <AnimatePresence>
        {lightbox && (
          ...
          >
            <button
              onClick={() => setLightbox(false)}
              className="absolute top-4 right-4 text-white bg-white/10 p-2 rounded-xl hover:bg-white/20"
            >
              <X size={24} />
            </button>
            ...
            <button
              ...
              className="absolute left-4 text-white bg-white/10 p-3 rounded-xl hover:bg-white/20"
            >
              <ChevronLeft size={24} />
            </button>
            ...
            <button
              ...
              className="absolute right-4 text-white bg-white/10 p-3 rounded-xl hover:bg-white/20"
            >
      ...
      {video?.url && (
        <div className="mt-4">
          <h4 className="font-semibold text-dark mb-3 flex items-center gap-2">
            <Play size={18} className="text-primary-600" /> Vidéo de présentation
          </h4>
          <video
            controls
...
export default function ApartmentDetailPage() {
  ...
  useEffect(() => {
    apartmentsAPI.getById(id)
      .then(r => {
        setApartment(r.data);
        document.title = `${r.data.title} - Lokea`;
      })
  ...
  const { title, description, location, pricePerNight, currency, rooms, bathrooms,
          maxGuests, amenities, images, video, isAvailable, views } = apartment;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid lg:grid-cols-3 gap-8 mt-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-4">
                {[
                  ...
                  <div key={label} className="bg-primary-50 px-4 py-2 rounded-xl flex items-center gap-2">
                    <span>{icon}</span>
            ...
            {amenities?.length > 0 && (
              <div className="bg-white rounded-2xl p-6 shadow-card">
                <h2 className="text-xl font-bold text-dark mb-4">Équipements inclus</h2>
            ...
            {/* Tarif */}
            <div className="bg-white rounded-2xl p-6 shadow-card">
              <h2 className="text-xl font-bold text-dark mb-4">Tarif</h2>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-primary-600">
                  {new Intl.NumberFormat('fr-FR').format(pricePerNight)}
                </span>
                <span className="text-gray-500">{currency} / nuit</span>
              </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24">
              ...
              {/* Contact direct */}
              <div className="mt-4 bg-white rounded-2xl p-5 shadow-card text-center">
