// =============================================
// MD SERVICE - Page Détail Voiture (Dark Mode)
// =============================================
...
function CarGallery({ images, video, title }) {
  ...
  if (total === 0) return (
    <div className="w-full h-72 bg-gray-200 dark:bg-gray-700 rounded-2xl
                    flex items-center justify-center">
      <Car size={48} className="text-gray-300 dark:text-gray-500" />
    </div>
  );
...
export default function CarDetailPage() {
  ...
  useEffect(() => {
    ...
    carsAPI.getById(id)
      .then(r => {
        if (r.data) { setCar(r.data); document.title = `${r.data.title || 'Véhicule'} - MD Service`; }
  ...
  if (error || !car) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4
      ...
      <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-card
                      border border-transparent dark:border-gray-700 text-center max-w-md">
        <Car size={48} className="mx-auto text-gray-300 dark:text-gray-600 mb-4" />
        <h2 className="text-xl font-bold text-dark dark:text-white mb-2">Oups !</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-6">{error || 'Véhicule introuvable'}</p>
        <Link to="/voitures"
          className="inline-flex items-center gap-2 bg-primary-600 text-white
                     px-6 py-3 rounded-xl font-semibold hover:bg-primary-700 transition-colors">
  ...
  const { title, brand, model, year, description, pricing, currency,
          transmission, fuel, seats, color, features, images, video, isAvailable } = car;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <ShareButton 
              title={title} 
              text={`Regarde ce véhicule sur MD Service : ${title} (${brand})`} 
            />
            <FavoriteButton 
              item={car} 
              type="car" 
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex flex-col sm:flex-row items-start justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium">
                    {brand} {model}
                    {year && <><span className="mx-1">•</span>{year}</>}
                    {color && <><span className="mx-1">•</span>{color}</>}
                  </p>
                </div>
                ...
                {[
                  ...
                  <div key={label}
                    className="bg-gray-50 dark:bg-gray-700/50 rounded-2xl p-4
                               ...
                               hover:border-primary-100 dark:hover:border-primary-800 transition-colors">
                    <Icon size={22} className="text-primary-600 dark:text-primary-400 mb-2" />
                    <p className="text-[11px] uppercase tracking-wider
                                  text-gray-400 dark:text-gray-500 font-bold mb-1">{label}</p>
                    <p className="text-sm font-bold text-dark dark:text-white">{val || 'N/A'}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {pricingRows.length > 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-card
                ...
                <p className="text-gray-400 dark:text-gray-500 text-[11px] mt-4 italic font-medium">
            ...
            {features?.length > 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-card
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-6">
                  {features.map(f => (
                    <div key={f} className="flex items-center gap-3 text-gray-700 dark:text-gray-300 py-1">
                      <div className="w-5 h-5 rounded-full bg-green-100 dark:bg-green-900/30
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-4">
              <div className="bg-white dark:bg-gray-800 rounded-3xl p-2 shadow-xl
                ...
                {car && car.id && <BookingForm listing={car} type="car" />}
              </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-card
                ...
                <a
                  href={getWhatsAppUrl(
                    `Bonjour MD Service, je souhaite réserver la ${title} (${brand} ${model} ${year}). Est-elle disponible ?`
                  )}
                  target="_blank" rel="noreferrer"
                  className="inline-flex items-center justify-center gap-3 w-full
                             bg-[#25D366] text-white py-4 rounded-xl font-bold
                             hover:shadow-lg hover:shadow-green-200 dark:hover:shadow-green-900/30
