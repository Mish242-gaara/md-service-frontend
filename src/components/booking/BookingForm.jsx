// =============================================
// LOKEA - Formulaire de Réservation (CORRIGÉ SEQUELIZE)
// =============================================
...
export default function BookingForm({ listing, type }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Préparation des données pour correspondre aux colonnes Sequelize
      const reservationData = {
        type: type, // 'car' ou 'apartment'
        listingId: parseInt(listing.id || listing._id),
        listingModel: type === 'car' ? 'Car' : 'Apartment',
        listingTitle: listing.title,
        
        // MAPPING CRITIQUE : Correspondance avec les colonnes clientName/Phone/Email
...
${form.message ? `💬 Message : ${form.message}` : ''}`;
  ...
  return (
    <div className="bg-white rounded-2xl shadow-card p-6">
      <h3 className="text-xl font-bold text-dark mb-1">Réserver maintenant</h3>
      <p className="text-gray-500 text-sm mb-6">Réponse garantie sous 1h</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {totalInfo && (
          ...
          >
            <div className="flex items-center justify-between text-sm text-gray-600 mb-1">
              <span>{formatPrice(pricePerDay, listing.currency)} × {totalInfo.days} jour{totalInfo.days > 1 ? 's' : ''}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="font-bold text-dark">Total estimé</span>
        ...
        >
          ...
          {!listing.isAvailable ? 'Déjà réservé' : loading ? 'Envoi...' : 'Envoyer ma demande'}
        </button>
